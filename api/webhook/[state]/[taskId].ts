...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Validate known states to avoid arbitrary loops
  const allowedStates = new Set(["translate", "verify", "review", "finalize"]);
  if (!allowedStates.has(state)) {
    return res
      .status(400)
      .json({ error: "Bad Request", message: "Invalid state path" });
  }
  ...
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: `State(${state}) processing started for ${taskId}`,
      });
