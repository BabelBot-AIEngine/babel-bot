...
export class ProlificService {
  ...
  async setupAndWaitForBatch(
    batchId: string,
    datasetId: string,
    tasksPerGroup: number = 1
  ): Promise<void> {
    console.log(
      `Setting up batch ${batchId} with dataset ${datasetId} and ${tasksPerGroup} tasks per group...`
    );
  ...
  async setupBatch(
    ...
    const response = await this.makeRequest(
      `/data-collection/batches/${batchId}/setup`,
      {
        method: "POST",
  ...
  private async waitForBatchReady(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      console.log(
        `Batch ${batchId} status: ${statusResponse.status}, waiting...`
      );
    ...
    throw new Error(
      `Batch ${batchId} did not become ready within ${
        (maxAttempts * intervalMs) / 1000
      } seconds`
    );
  }
  ...
  async createParticipantGroup(
    participantGroupData: CreateParticipantGroupRequest
  ...
  async getAllParticipantGroups(
    ...
    try {
      ...
      if (!wsId) {
        throw new Error(
          "workspace_id is required for getting participant groups"
        );
      }

      const response = await this.makeRequest(
        `/participant-groups/?workspace_id=${wsId}`
      );
      return response.results || [];
    } catch (error) {
  ...
  async ensureInternalParticipantGroupExists(): Promise<ParticipantGroup | null> {
    ...
    const participantIds = internalParticipants
      .split(",")
      .map((id) => id.trim())
    ...
    if (participantIds.length === 0) {
      console.warn(
        "INTERNAL_PARTICIPANTS is set but contains no valid participant IDs"
      );
      return null;
    }
    ...
    if (!workspaceId) {
      console.error(
        "PROLIFIC_WORKSPACE_ID is required to create participant groups"
      );
      return null;
    }
    ...
    try {
      ...
      if (existingGroup) {
        console.log(
          `Using existing internal participant group: ${groupName} (${existingGroup.id})`
        );
        return existingGroup;
      }

      console.log(
        `No existing participant group found with name: ${groupName}`
      );
    } catch (error) {
    ...
    try {
      console.log(
        `Creating internal participant group with ${participantIds.length} participants in workspace ${workspaceId}`
      );
      ...
      console.log(
        `Created internal participant group: ${groupName} (${newGroup.id})`
      );
      return newGroup;
    } catch (error) {
  ...
  async getAllFilters(): Promise<ProlificFilterDefinition[]> {
    try {
      const response: ProlificFiltersResponse = await this.makeRequest(
        "/filters/"
      );
      return response.results || [];
    } catch (error) {
  ...
  async getLanguageFilters(targetLanguages: string[]): Promise<StudyFilter[]> {
    try {
      console.log(
        `Fetching language filters for: ${targetLanguages.join(", ")}`
      );
      ...
      for (const targetLanguage of targetLanguages) {
        ...
        console.log(
          `Searching for language filters for ${targetLanguage} using terms: ${searchTerms.join(
            ", "
          )}`
        );
        ...
        const matchingFilters = allFilters.filter((filter) => {
          ...
          return searchTerms.some((term) => {
            ...
            const choiceMatches =
              filter.choices &&
              Object.values(filter.choices).some((choice) => {
            ...
            return (
              filterIdMatches || titleMatches || tagMatches || choiceMatches
            );
          });
        });
        ...
        if (matchingFilters.length > 0) {
          console.log(
            `Found ${matchingFilters.length} potential language filters for ${targetLanguage}:`
          );
          matchingFilters.forEach((f) =>
            console.log(
              `  - ${f.filter_id} (${f.title}) - Category: ${f.category}, Type: ${f.type}`
            )
          );

          // Priority 1: Domain Expert filters
          const domainExpertFilter = matchingFilters.find(
            (filter) =>
          ...
          if (domainExpertFilter) {
            console.log(
              `Using domain expert filter: ${domainExpertFilter.filter_id}`
            );
            ...
            if (domainExpertFilter.choices) {
              ...
              console.log(
                `Added domain expert filter for ${targetLanguage}: ${
                  domainExpertFilter.filter_id
                } with values: ${selectedValues.join(", ")}`
              );
            } else {
              console.warn(
                `Domain expert filter ${domainExpertFilter.filter_id} has no choices available`
              );
            }
          } else {
            // Priority 2: Test score filters (fallback)
            const testScoreFilter = matchingFilters.find(
              (filter) =>
                filter.type === "range" &&
                (filter.data_type === "float" ||
                  filter.data_type === "integer") &&
                (filter.filter_id.includes("test") ||
                  filter.filter_id.includes("score"))
            );

            if (testScoreFilter) {
              console.log(
                `Using test score filter: ${testScoreFilter.filter_id} (min: ${testScoreFilter.min}, max: ${testScoreFilter.max})`
              );
              ...
              console.log(
                `Added test score filter for ${targetLanguage}: ${testScoreFilter.filter_id} with range: ${minScore}-${maxScore}`
              );
            } else {
              // Priority 3: General language fluency filters (existing logic)
              const generalFilter = matchingFilters.find(
                (filter) => filter.choices
              );

              if (generalFilter) {
                console.log(
                  `Using general language filter: ${generalFilter.filter_id}`
                );
                console.log(
                  `Filter choices available: ${JSON.stringify(
                    generalFilter.choices
                  )}`
                );

                // Look for fluency levels - prefer "fluent", "native", or "advanced"
                const fluencyKeys = Object.keys(generalFilter.choices!).filter(
                  (key) => {
                    const value = generalFilter.choices![key].toLowerCase();
                    return (
                      value.includes("fluent") ||
                ...
                console.log(
                  `High fluency keys found: ${fluencyKeys.join(", ")}`
                );

                // If no specific fluency levels found, use all available options
                const selectedValues =
                  fluencyKeys.length > 0
                    ? fluencyKeys
                    : Object.keys(generalFilter.choices!);
                ...
                if (selectedValues.length > 0) {
                  ...
                  console.log(
                    `Added general language filter for ${targetLanguage}: ${
                      generalFilter.filter_id
                    } with values: ${selectedValues.join(", ")}`
                  );
                } else {
                  console.warn(
                    `No valid choices found for general language filter: ${generalFilter.filter_id}`
                  );
                }
              } else {
                console.warn(
                  `No usable language filters found for ${targetLanguage}`
                );
              }
            }
          }
          ...
          console.warn(
            `No language filters found for ${targetLanguage}. Available filter IDs: ${allFilters
              .slice(0, 5)
              .map((f) => f.filter_id)
      ...
      console.log(
        `Final language filters: ${languageFilters.length} filters created`
      );
  ...
  async getStudyFilters(targetLanguages: string[]): Promise<StudyFilter[]> {
    ...
    if (participantGroupFilters) {
      return participantGroupFilters;
    }

    return await this.getLanguageFilters(targetLanguages);
  }
